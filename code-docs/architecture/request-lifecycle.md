...
- **Deployment environments** — `config.environments` declares the app's environments (`url`, `cron.secret`/`cron.enabled`, `email.filter`) and the build step `buildEnvironments` (run from `validateConfig`) resolves the current one from `config.environment` or `LOWDEFY_ENVIRONMENT` into `config.environment`, converts the deprecated 6.0 `config.cron.environments` shape (its url-less host becomes the current environment when none is set), and defaults `logger.sentry.environment`. At runtime `getCurrentEnvironment({ config })` returns `{ name, ...settings }` or null; it is passed to every connection resolver as `environment` (SMTP/SendGrid fall back to its `email.filter`), defaults the `RenderNotification` `serverUrl`, and `getCanonicalUrl` (BetterAuth base URL, MCP resource URIs) falls back to its `url` when `BETTER_AUTH_URL` is unset.
