...
Lowdefy scans YAML config at build time to extract all Tailwind class names:

```
YAML config  →  collectTailwindClasses()  →  tailwind-classes.js  →  Tailwind JIT
```

**File:** `packages/build/src/build/collectTailwindClasses.js`

The collector walks every block in the page tree, recursing into `blocks`, `areas`, and `slots`. It extracts class values from strings, arrays, and objects:
...
The collected classes are written to `tailwind-classes.js`:

```javascript
// Generated by Lowdefy build
export default 'p-4 text-primary bg-bg-container rounded-lg flex gap-4 ...';
```

The `globals.css` declares this file as a Tailwind source:
...
/* Trigger file for CSS recompilation */
...
In development, the `tailwind-candidates.css` file is rewritten on every page change. This triggers PostCSS to re-run, which causes Tailwind to re-scan all `@source` paths. This ensures newly added Tailwind classes are compiled immediately.

The `tailwind-classes.js` file is also regenerated during dev rebuilds with the latest collected classes from YAML config.
...
| File                                                                        | Purpose                                                       |
| --------------------------------------------------------------------------- | ------------------------------------------------------------- |
| `packages/build/src/build/writePluginImports/writeGlobalsCss.js`            | Generates `globals.css` with layer order, bridge, and sources |
| `packages/build/src/build/collectTailwindClasses.js`                        | Extracts Tailwind classes from YAML config                    |
...
### Why Collect Classes at Build Time?

Tailwind v4 uses JIT compilation — it only generates CSS for classes it finds in source files. Since Lowdefy config is YAML (not JS/HTML), Tailwind can't scan it natively. The build extracts all class strings and writes them to `tailwind-classes.js`, which Tailwind scans via `@source`.
