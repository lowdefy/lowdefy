...
- Chart labels render in Roboto again. pdfmake's SVG font callback hands svg-to-pdfkit the raw font descriptor, and pdfkit tried to open the virtual-filesystem name from disk, so every `<text>` fell back to Helvetica with a `failed to open font` warning. Fonts are now pdfmake's bundled Roboto files by absolute path, which both code paths open and pdfkit caches.
