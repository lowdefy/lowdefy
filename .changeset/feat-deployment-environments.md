...
- **`url`** — the `RenderNotification` `serverUrl` (email links and logos; on the dev server, with no environment url, the request origin) and the auth base URL (`AUTH_URL`) default to it; an explicit value still wins.
