...
- **`url`** — the `RenderNotification` `serverUrl` (email links and logos), the auth canonical URL (`BETTER_AUTH_URL`: auth links, CSRF origin allowlist) and the MCP resource URIs default to it; an explicit value still wins.
