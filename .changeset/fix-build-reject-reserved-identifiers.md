...
fix(build): Reject reserved names as agent ids, agent tool names, locale codes and event shortcuts.

Each of these author-written identifiers later becomes a key in a plain object — the sub-agent graph
and agent registry, the agent `tools` map, the i18n message catalogs and the client's shortcut map. A
reserved name such as `__proto__` or `constructor` resolved through `Object.prototype` instead of
adding an entry, so the config built clean and misbehaved later: a duplicate id went undetected, or the
build crashed with an unlocated internal error. Only the agent `tools` map already failed loudly, with
a bare `ReservedKeyError` from `setKey` at runtime. None of these sites had a build-time shape check,
