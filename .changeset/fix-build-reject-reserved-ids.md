...
fix(build): Reject reserved names as page, request, connection, websocket, notification, endpoint and
step ids.
...
adding an entry — a build-clean config that fails at runtime. `validateId` now rejects the seven
