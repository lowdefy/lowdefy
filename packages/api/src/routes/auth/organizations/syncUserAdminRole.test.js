...
function registerPinnedBinding({ auth, pinnedId = 'org_pinned' }) {
  ...
  setPinnedOrganization({
    auth,
    organization: { id: pinnedId, slug: 'org-a', name: 'Org A' },
    slug: 'org-a',
  });
}
...
test('syncUserAdminRole writes user.role when the pinned member roles include the user-admin role', async () => {
  ...
  registerPinnedBinding({ auth });

  await syncUserAdminRole({ auth, userAdminRole: 'user-admin', userId: 'user_1' });
...
test('syncUserAdminRole does nothing when userAdminRole is not configured', async () => {
  ...
  await syncUserAdminRole({ auth, userAdminRole: null, userId: 'user_1' });
...
test('syncUserAdminRole throws when the pinned organization is not resolved', async () => {
  ...
  await expect(
    syncUserAdminRole({ auth, userAdminRole: 'user-admin', userId: 'user_1' })
  ).rejects.toThrow(
