...
describe('error input', () => {
  ...
  test('PluginError does not log stack trace', async () => {
    const { PluginError } = await import('@lowdefy/errors');
    const { default: createCliLogger } = await import('./createCliLogger.js');
    const logger = createCliLogger({ logLevel: 'info' });
    const err = new PluginError({ message: 'plugin broke' });
    logger.error(err);
    expect(mockOraFail.mock.calls.length).toBe(1);
    expect(mockOraFail.mock.calls[0][0]).toBe('\x1b[31m[PluginError] plugin broke\x1b[0m');
  });
