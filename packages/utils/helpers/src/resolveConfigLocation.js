 ...
 * @returns {Object|null} Location object with path, file, and formatted string, or null if not resolvable
 *
 * @example
 * const location = resolveConfigLocation({
 *   configKey: 'abc123',
 *   keyMap: { 'abc123': { key: 'root.pages[0:home].blocks[0:header]', '~r': 'ref1' } },
 *   refMap: { 'ref1': { path: 'pages/home.yaml' } }
 * });
 * // Returns: { path: 'root.pages[0:home].blocks[0:header]', file: 'pages/home.yaml', formatted: 'pages/home.yaml at root.pages[0:home].blocks[0:header]' }
 */
function resolveConfigLocation({ configKey, keyMap, refMap }) {
  ...
  return {
    path: keyEntry.key,
    file: filePath,
    formatted: `${filePath} at ${keyEntry.key}`,
  };
}
