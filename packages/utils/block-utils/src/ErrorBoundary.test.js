...
test('attaches configKey to error when provided', () => {
...
test('does not override existing configKey on error', () => {
  let capturedError = null;
  const ProblemChild = () => {
    const error = new Error('ErrorBoundary test error');
    error.configKey = 'existing-config-key';
    throw error;
  };
  render(
    <ErrorBoundary
  ...
  expect(capturedError).not.toBeNull();
  expect(capturedError.configKey).toBe('existing-config-key');
});
