...
const runInstance = ({ meta, methodName, operator, params, instanceType }) => {
  if (!meta[methodName]) {
    throw new Error(
  ...
  if (type.typeOf(instance) !== instanceType) {
    throw new Error(
      `${operator}.${methodName} must be evaluated on an ${instanceType} instance. For named args provide an ${instanceType} instance to the "on" property, for listed args provide and ${instanceType} instance as the first element in the operator argument array.`
    );
  }
