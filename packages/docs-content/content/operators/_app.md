...
The metadata object has the following keys:
  ...
  - `buildId`: A random identifier generated by each build, so two builds never share one. The production server sends it with every page config fetched during in-app navigation, and the client bundle carries the id of the build it came from; when the two differ the tab reloads once to pick up the new bundle instead of failing on functions or plugins it does not have. Useful in config as a cache-busting value that changes on every deploy.
...
_app:
  all: true
```
Returns: The full `{ slug, name, version, description, license, lowdefyVersion, gitSha, buildId }` object.
