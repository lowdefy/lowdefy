...
`response` is the value the action returned. For a [`CallAPI`](/CallAPI) action that is the endpoint's `:return` value, so the endpoint's data is read at `_actions.<id>.response.<path>`; the call's `status`, `success` and `responseTime` are read with [`_api`](/_api). Before v8 `CallAPI` returned the whole api record, putting the endpoint's data at `_actions.<id>.response.response.<path>`. The v8 build rewrites that spelling and warns (check slug `actions-response-envelope`); it is removed in v9.
