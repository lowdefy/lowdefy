...
Lowdefy has built-in semantic icon names such as `edit`, `delete`, `warning` and `external-link`. Use them anywhere an icon name is accepted, and in HTML with `data-icon`:
...
The built-in names map to [Lucide](https://lucide.dev) icons, so an app that uses them looks consistent. Any [React Icons](https://react-icons.github.io/react-icons/) name, like `AiOutlineUser` or `LuPencil`, still works everywhere.

Built-in names: `add`, `arrow-down`, `arrow-left`, `arrow-right`, `arrow-up`, `attach`, `bell`, `calendar`, `chart`, `check`, `chevron-down`, `chevron-left`, `chevron-right`, `chevron-up`, `clock`, `close`, `copy`, `delete`, `document`, `download`, `drag`, `edit`, `error`, `external-link`, `file`, `filter`, `folder`, `globe`, `grid`, `heart`, `help`, `hide`, `history`, `home`, `image`, `info`, `link`, `list`, `loading`, `location`, `lock`, `login`, `logout`, `mail`, `menu`, `message`, `minus`, `more`, `more-vertical`, `phone`, `print`, `refresh`, `save`, `search`, `send`, `settings`, `share`, `sort`, `star`, `success`, `tag`, `unlock`, `upload`, `user`, `users`, `view`, `warning`.

Ant Design icon names also work and map to the same icons: `plus`, `setting`, `reload`, `ellipsis`, `printer`, `info-circle`, `check-circle`, `close-circle`, `clock-circle`, `file-text`, `environment`, `left`, `right`, `up` and `down`.

Add your own names, or point a built-in name at a different icon, with `theme.icons.aliases`. Names are lowercase kebab-case; targets are React Icons names:
...
## TLDR
  ...
  - Use built-in semantic icon names (`icon: edit`), and `data-icon` in HTML; add names with `theme.icons.aliases`. See [HTML attributes](/html-attributes) for everything HTML understands.
