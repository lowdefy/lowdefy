...
- `environment: string`: The name of the environment this build is for. Defaults to the `LOWDEFY_ENVIRONMENT` environment variable. When `environments` is declared it must be one of its names. Sentry reports under this name unless `logger.sentry.environment` is set.
- `cron: object`: **Deprecated** — declare the environments under `environments` instead. The 6.0 shape `cron.environments` (one environment without a `url` is the deployment Vercel fires crons on; the others have `url`, `secret` and `enabled`) still builds, with a warning.
