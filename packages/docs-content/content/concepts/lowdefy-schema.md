...
| `collections` | The [`collections:`](/collections) declaration - tenancy agreement between a collection and its connections, and the check-only notes on undeclared, operator-named and untenanted connections |
| `types` | All type validation - unknown blocks, operators, actions, requests, connections, steps, agents, notifications and websockets. Since v8 an unknown operator is a build error, so this slug is what suppresses a key that only looks like an operator. |
| `events` | Block event name validation |
| `block-properties` | Block `properties` validation against the block type's schema (unknown or mistyped properties) |
| `schema` | JSON schema validation errors |
| `state-schema` | Page `state:` contract violations - a block id, `SetState` key or `_state` path the declared contract does not cover, or a fragment that is not a valid JSON schema |
| `js-lint` | `_js` body analysis - a reference to a name that is not a parameter, not declared in the body and not in the JavaScript standard library is a build error; a declaration that is never used is a warning |
| `tenant` | Tenant wall audits on requests and steps over a walled connection - an authored tenant field is a build error; the `tenant: none` audits (no tenant field, a caller-supplied tenant value, an unstamped write) and the unscoped inventory run under `lowdefy check` |
| `js-modules` | `_js` module references (`fn: ./lib/file.js#name`) - the file must exist inside the config directory, parse, and have the named export |
| `response-schema` | Endpoint `responseSchema` checks - a `_actions.<id>.response.response.<path>` or `_step.<id>.<path>` read that the target endpoint's `responseSchema` does not declare, and an endpoint `payloadSchema` or `responseSchema` that is not a valid JSON Schema |
| `event-payload` | `_event` paths checked against the payload schema the block type declares for that event (`meta.events.<name>.payload`) - a path the payload has no room for is a build error; events without a declared payload are never checked |
