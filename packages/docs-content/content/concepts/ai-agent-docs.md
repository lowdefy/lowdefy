...
Everything is served under the `/docs` path of your dev server (default `http://localhost:3000`). No setup or configuration is needed — it is always on in dev, and never part of your production server.

> The `/docs` route prefix is reserved by the dev server. A page with `id: docs` will not be reachable in dev.
...
```bash
claude mcp add --transport http lowdefy-docs http://localhost:3000/docs/mcp
```
...
`http://localhost:3000/docs` (also as MCP tools via the `lowdefy-docs` server).

Never guess type names or properties. Before writing config:

1. Call `lowdefy_list_types` (or `GET /docs/blocks`, `/docs/operators`,
   `/docs/actions`, `/docs/connections`, `/docs/requests`) to find the exact
   type name — this includes this project's local plugins.
2. Call `lowdefy_get_schema` (or `GET /docs/schema/{kind}/{type}`) for the
   exact properties and events of that type.
3. Call `lowdefy_get_examples` (or `GET /docs/examples/{type}`) to see real
...
| Route                       | Purpose                                                        |
| --------------------------- | -------------------------------------------------------------- |
| `GET /docs`                 | Overview and route index                                        |
| `GET /docs/{kind}`          | List all available types of a kind, e.g. `/docs/blocks`         |
| `GET /docs/plugins`         | Installed plugin packages and the types each provides           |
| `GET /docs/schema/{kind}/{type}` | JSON schema for a type, e.g. `/docs/schema/blocks/Button` |
| `GET /docs/examples/{type}` | Example YAML for a block type                                   |
| `GET /docs/content/{slug}`  | A docs page as markdown, e.g. `/docs/content/operators/_get`    |
| `GET /docs/search?q={query}`| Search the docs                                                 |
...
- Types appear in `lowdefy_list_types` and `/docs/{kind}` listings.
