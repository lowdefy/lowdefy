...
When the connection sets no `filter`, the current environment's `email.filter` from [`config.environments`](/lowdefy-schema) applies instead, so the per-environment rule lives in one place for every mail connection. A connection's own `filter` always wins. Auth emails are never filtered.
