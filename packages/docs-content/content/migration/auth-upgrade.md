...
| Roles | On the user record via `userFields` | App roles on `member.appRoles` (`_user.roles`); the organization's `owner`/`admin`/`member` tier on `member.role` (`_user.orgRoles`) |
...
The session user carries `id`, `name`, `email`, `image`, `emailVerified`, `activeOrganizationId`, and the user record's own `profile` bag, plus three fields Lowdefy resolves from the caller's row in the active organization:
