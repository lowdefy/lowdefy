...
The `_format` operator has been replaced with the [`_intl`](/_intl) and [`_moment`](/_moment) operator, with some minor schema changes. Note: `_moment` was renamed to [`_dayjs`](/_dayjs) in v5.
...
- The callback URLs configured with the provider change from `/auth/openid-callback` to `/api/auth/callback/`.
