...
test('parse validate on fields', async () => {
  ...
  context._internal.RootSlots.validate(match);
...
test('RootBlock.validate(match) to ignore errors where field not visible', async () => {
  ...
  const { text, list } = context._internal.RootSlots.map;
...
test('showValidation only on fields that matches for error', async () => {
  ...
  const { text1, text2 } = context._internal.RootSlots.map;
  ...
  context._internal.RootSlots.validate((id) => id === 'text1');
...
test('drop showValidation on RootSlots.reset()', async () => {
  const pageConfig = {
    id: 'root',
    type: 'Box',
    events: {
...
test('drop showValidation on RootSlots.resetValidation()', async () => {
  const pageConfig = {
    id: 'root',
    type: 'Box',
    events: {
  ...
  expect(text1.showValidation).toBe(false);
  expect(text2.showValidation).toBe(false);
  context._internal.RootSlots.validate((blockId) => blockId === 'text1');
  expect(text1.showValidation).toBe(true);
  expect(text2.showValidation).toBe(false);
  context._internal.RootSlots.resetValidation(() => false);
  expect(text1.showValidation).toBe(true);
  expect(text2.showValidation).toBe(false);
  context._internal.RootSlots.resetValidation((blockId) => blockId === 'text1');
  expect(text1.showValidation).toBe(false);
  expect(text2.showValidation).toBe(false);
});
