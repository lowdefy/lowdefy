...
function getOverview() {
  ...
  const lines = [
    ...
    '   Or subscribe instead of polling: `GET /lowdefy-docs/events` streams `build`, `client_error`, `server_error`, `restart` and `fixture_seeded` events over SSE (MCP clients receive the same as `notifications/message` from logger `lowdefy`).',
    ...
    '4. **Verify behaviour**: `POST /lowdefy-docs/journey` `{pageId, steps, user, urlQuery}` — or `lowdefy_run_journey`. Drive the page headless with declarative steps (`click`, `fill`, `select`, `press`, `wait`, `screenshot`, `expect: { state | visible | text | url }`, blocks addressed by blockId) and assert what happens. A failing step stops the journey and returns `passed: false` with `failure: { index, step, expected, actual, message }` and the final page `state` — data, not an error.',
    '5. **Locate config**: `GET /lowdefy-docs/find/{id}?pageId=...` — or `lowdefy_find_config`. Which yaml file defines a page/block/request id.',
    '6. **Scaffold**: `lowdefy_scaffold_page` creates a canonical new page file (then register it in lowdefy.yaml).',
    '',
    ...
    '4. **Run an endpoint**: `POST /lowdefy-docs/run-endpoint` `{endpointId, payload, user}` — or `lowdefy_run_endpoint`. Run an Api routine headlessly and see what it returns, rejects or throws — a `:reject` comes back as `status: "reject"` with the routine\'s own error, not as a failure. Always needs `cli.agentTools.allowWriteRequests: true` (routines are not classified read-only).',
    '5. **Brief yourself on a page**: `GET /lowdefy-docs/app-brief/{pageId}` (or `/app-brief` for the whole app, `?since={ref}` for a diff) — or `lowdefy_app_brief`. What the page reads and writes (its requests and the endpoints it calls), the journeys and request tests covering it, the declared `(blockId, event)` triples nothing tests, and what changed since a git ref. Read this before opening config files.',
    '6. **Understand the app**: `GET /lowdefy-docs/app-map` — or `lowdefy_app_map`. Every page, menu, connection, endpoint, agent, and websocket in one call.',
    '7. **Understand the data**: `GET /lowdefy-docs/data-model` — or `lowdefy_data_model`. Every collection with its fields, relations, indexes and tenant verdict, the connections addressing it, and every request, routine step and websocket that reads or writes it (with source file:line). Anything that could not be joined is under `unresolved`.',
    "8. **Seed data**: `POST /lowdefy-docs/seed-fixture` `{name, reset}` — or `lowdefy_seed_fixture`. Load `fixtures/<name>.yaml` (documents keyed by connectionId, the same files request tests use) into the dev database through the connection layer so a page has data while you build it. Needs `cli.agentTools.allowWriteRequests: true`; `reset: true` empties the fixture's collections first (default layers onto existing data); documents are never tenant-stamped. Emits a `fixture_seeded` push event.",
    '9. **Config checkpoints**: `POST /lowdefy-docs/checkpoints` `{label}` / `POST /lowdefy-docs/checkpoints/revert` `{id}` — or `lowdefy_checkpoint` / `lowdefy_revert_checkpoint`. Snapshot config files before risky changes; revert instantly.',
    "10. **State checkpoints**: `POST /lowdefy-docs/state-checkpoints/snapshot` `{pageId, name}` captures live page state + request responses into .lowdefy/state-checkpoints/<name>/ (gitignored — checkpoints contain user/session data); `POST /lowdefy-docs/state-checkpoints/load` `{name, mode}` restores it — 'registry-only' mode returns a ?_checkpoint URL a human can open to test the app in that exact state. While a checkpoint is loaded with replay on (the default), page requests are answered from the checkpoint instead of the database until the next build.",
    "11. **Restart the server**: `POST /lowdefy-docs/restart` `{reason}` — or `lowdefy_restart`. After editing a local plugin's server-side implementation, or when build-status looks stale. Answers before the restart lands: wait ~2s, then poll `GET /lowdefy-docs/build-status`. Local plugin server-side sources are also watched and restart the server automatically.",
    '',
    ...
    '| `GET /lowdefy-docs/events` | SSE push channel: `restart` on connect, then `build`, `client_error`, `server_error`, `fixture_seeded` events as they happen (build events carry `stale`) |',
    ...
    '| `POST /lowdefy-docs/run-endpoint` | Execute an Api endpoint routine with a test payload and caller (needs agent write access; rejects return as data) |',
    "| `POST /lowdefy-docs/seed-fixture` | Load `fixtures/{name}.yaml` into the dev database through the connection layer (`{name, reset}`; needs agent write access; `reset` empties the fixture's collections first) |",
    '| `GET /lowdefy-docs/app-brief[/{pageId}]?since={ref}` | Deterministic brief: what a page reads and writes (requests + the endpoints it calls), the journeys and request tests covering it, its uncovered `(blockId, event)` triples, and what a git diff since `{ref}` changed |',
    '| `GET /lowdefy-docs/app-map` | Whole-app graph: pages, menus, connections, endpoints, agents |',
    '| `GET /lowdefy-docs/data-model` | Data layer: collections, fields, relations, tenant verdicts, connections, and which requests/steps/websockets read or write each (with `unresolved`) |',
    '| `GET /lowdefy-docs/ops/errors?since={deploy|ISO}&group_by={source|org|page|endpoint}` | Production errors from the log sink, grouped, each with `source` resolved from the running build (locked: read-only sink credentials, loopback only, `config.ops.enabled`) |',
