...
function getOverview() {
  ...
  const lines = [
    ...
    '6. **Icons**: `GET /lowdefy-docs/icons?q=...` — or `lowdefy_search_icons`. Prefer semantic names (`icon: edit`); in HTML use `<i data-icon="edit"></i>`, `data-tooltip` and `data-popover`.',
    ...
    '',
    '1. **Check what broke**: `GET /lowdefy-docs/build-status?wait=true` — or `lowdefy_build_status` with `wait: true`, which answers once your latest edit has been processed and the pages it touched have been built. Current build errors and warnings with source file locations, the pages that fail to build (`pages.failed`), plus recent browser runtime errors. Errors from before the latest edit come apart under `earlierErrors`.',
    ...
    "3. **See it rendered**: `GET /lowdefy-docs/screenshot/{pageId}` — or `lowdefy_screenshot_page`. PNG of the page in headless Chromium; `?viewportWidth=390&colorScheme=dark` (the tool's `width` and `colorScheme`) checks a phone layout in dark mode.",
    '4. **Verify behaviour**: `POST /lowdefy-docs/journey` `{pageId, steps, user, urlQuery, state}` — or `lowdefy_run_journey`. Drive the page headless with declarative steps (`click`, `fill`, `select`, `press`, `back`, `wait`, `screenshot`, `expect: { state | visible | text | url | title }`) and assert what happens. Targets are a blockId, or `{ blockId, row, column, text, nth }` to reach the button in a grid cell, a control by its exact text, or — with `text` alone — a confirm dialog / modal footer button rendered outside every block. A failing step stops the journey and returns `passed: false` with `failure: { index, step, expected, actual, message }` and the final page `state` — data, not an error. A final state over 10000 characters comes back as `stateOmitted` (its size and top-level keys); pass `state: [paths]` for the paths you need, `true` for all of it or `false` for none.',
    ...
    '3. **Run a request**: `POST /lowdefy-docs/run-request` `{pageId, requestId, payload}` — or `lowdefy_run_request`. Verify the data shape a request returns. The page is rebuilt first when it changed since its last build. Read-only types always run; writes need `cli.agentTools.allowWriteRequests: true` in lowdefy.yaml. A response over 40,000 characters is written in full to `responseFile` under `.lowdefy/responses/`; add `saveResponse: true` to always write it there.',
    '4. **Run an endpoint**: `POST /lowdefy-docs/run-endpoint` `{endpointId, payload, user}` — or `lowdefy_run_endpoint`. Run an Api routine headlessly and see what it returns, rejects or throws — a `:reject` comes back as `status: "reject"` with the routine\'s own error, not as a failure. Always needs `cli.agentTools.allowWriteRequests: true` (routines are not classified read-only). Add `system: true` to run a scheduled or detached-only `InternalApi` routine as a system context (no `_user`, auth not checked), the way cron does — not combinable with `user`. A large response is written to `responseFile` the same way as for a request, and `saveResponse: true` always writes it there.',
    ...
    '| `GET /lowdefy-docs/icons?q={query}` | Search icon names (semantic names first, then react-icons) |',
    ...
    '| `POST /lowdefy-docs/journey` | Drive a page headless through declarative steps (click, fill, select, press, back, wait, screenshot, expect) and assert state, visibility, text, url or title; screenshots returned as base64 |',
    ...
    '| `POST /lowdefy-docs/run-endpoint` | Execute an Api endpoint routine with a test payload and caller, or `system: true` for a cron-style system run (needs agent write access; rejects return as data) |',
