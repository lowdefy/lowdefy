...
function getOverview() {
  ...
  const lines = [
    ...
    "8. **Restart the server**: `POST /lowdefy-docs/restart` `{reason}` — or `lowdefy_restart`. After editing a local plugin's server-side implementation, or when build-status looks stale. Answers before the restart lands: wait ~2s, then poll `GET /lowdefy-docs/build-status`. Local plugin server-side sources are also watched and restart the server automatically.",
    '',
