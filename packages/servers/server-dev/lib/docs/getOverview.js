...
function getOverview() {
  ...
  const lines = [
    ...
    "7. **Seed data**: `POST /lowdefy-docs/seed-fixture` `{name, reset}` — or `lowdefy_seed_fixture`. Load `fixtures/<name>.yaml` (documents keyed by connectionId, the same files request tests use) into the dev database through the connection layer so a page has data while you build it. Needs `cli.agentTools.allowWriteRequests: true`; `reset: true` empties the fixture's collections first (default layers onto existing data); documents are never tenant-stamped. Emits a `fixture_seeded` push event.",
    '8. **Config checkpoints**: `POST /lowdefy-docs/checkpoints` `{label}` / `POST /lowdefy-docs/checkpoints/revert` `{id}` — or `lowdefy_checkpoint` / `lowdefy_revert_checkpoint`. Snapshot config files before risky changes; revert instantly.',
    "9. **State checkpoints**: `POST /lowdefy-docs/state-checkpoints/snapshot` `{pageId, name}` captures live page state + request responses into .lowdefy/state-checkpoints/<name>/ (gitignored — checkpoints contain user/session data); `POST /lowdefy-docs/state-checkpoints/load` `{name, mode}` restores it — 'registry-only' mode returns a ?_checkpoint URL a human can open to test the app in that exact state. While a checkpoint is loaded with replay on (the default), page requests are answered from the checkpoint instead of the database until the next build.",
    "10. **Restart the server**: `POST /lowdefy-docs/restart` `{reason}` — or `lowdefy_restart`. After editing a local plugin's server-side implementation, or when build-status looks stale. Answers before the restart lands: wait ~2s, then poll `GET /lowdefy-docs/build-status`. Local plugin server-side sources are also watched and restart the server automatically.",
    '',
