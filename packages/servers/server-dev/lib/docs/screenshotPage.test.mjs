...
// A real screenshot needs an installed browser, which jest can't rely on, so
// this only covers the error paths: bad input, and chromium.launch failing
// (the "no browser installed" case). chromium.launch is mocked so the real
// browser flow is untouched everywhere else.
...
// imports it) is loaded. Mirrors the pattern in docs.test.mjs.
...
test('screenshotPage returns an error when origin is missing', async () => {
  const result = await screenshotPage({ pageId: 'home' });
  expect(result.error).toMatch(/requires an "origin" string/);
});

test('screenshotPage returns an error when pageId is missing', async () => {
  const result = await screenshotPage({ origin: 'http://localhost:3001' });
