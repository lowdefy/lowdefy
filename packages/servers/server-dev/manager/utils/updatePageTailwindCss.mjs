...
async function updatePageTailwindCss({ changedFiles, context }) {
  ...
  if (contentWritten) {
    // Touch the candidates file so Vite's CSS pipeline re-runs Tailwind —
    // globals.css imports it, putting it in the dev module graph.
    const candidatesPath = path.join(context.directories.build, 'tailwind-candidates.css');
    fs.writeFileSync(
      candidatesPath,
