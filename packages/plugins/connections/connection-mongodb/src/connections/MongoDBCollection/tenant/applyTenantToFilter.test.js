...
test('returns bare equality for an undefined filter', () => {
  expect(applyTenantToFilter({ filter: undefined, tenant })).toEqual({ organizationId: 'org_a' });
});

test('returns bare equality for a null filter', () => {
  expect(applyTenantToFilter({ filter: null, tenant })).toEqual({ organizationId: 'org_a' });
});

test('returns bare equality for an empty filter', () => {
  expect(applyTenantToFilter({ filter: {}, tenant })).toEqual({ organizationId: 'org_a' });
});

test('merges the tenant equality into a filter with $and', () => {
  expect(applyTenantToFilter({ filter: { _id: 1 }, tenant })).toEqual({
    $and: [{ _id: 1 }, { organizationId: 'org_a' }],
  });
});
...
test('composes with a filter that already uses $and at the top level', () => {
  ...
  expect(applyTenantToFilter({ filter, tenant })).toEqual({
    $and: [{ $and: [{ a: 1 }, { b: 2 }] }, { organizationId: 'org_a' }],
  });
});
...
test('composes with a filter that uses $or at the top level', () => {
  ...
  expect(applyTenantToFilter({ filter, tenant })).toEqual({
    $and: [{ $or: [{ a: 1 }, { b: 2 }] }, { organizationId: 'org_a' }],
  });
});

test('throws when the tenant field is authored at the top level', () => {
  expect(() => applyTenantToFilter({ filter: { organizationId: 'org_b' }, tenant })).toThrow(
    'Tenant field "organizationId" can not be set in a filter on a tenant connection - the tenant wall stamps and filters it mechanically.'
  );
});

test('throws when the tenant field is authored nested in $or', () => {
  expect(() =>
    applyTenantToFilter({ filter: { $or: [{ a: 1 }, { organizationId: 'org_b' }] }, tenant })
  ).toThrow('Tenant field "organizationId" can not be set in a filter');
});

test('throws when a dotted tenant field key is authored', () => {
  expect(() => applyTenantToFilter({ filter: { 'organizationId.x': 1 }, tenant })).toThrow(
    'Tenant field "organizationId" can not be set in a filter'
  );
});

test('uses the position in the error message', () => {
  expect(() =>
    applyTenantToFilter({ filter: { organizationId: 'org_b' }, tenant, position: 'a query' })
...
test('uses the custom tenant field name', () => {
  ...
  expect(applyTenantToFilter({ filter: { organizationId: 'kept' }, tenant: customTenant })).toEqual(
    {
      $and: [{ organizationId: 'kept' }, { tenantId: 't_1' }],
    }
  );
