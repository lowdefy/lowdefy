...
// file rather than another case in createHtmlRenderer.mock.test.js, whose tests
// run in sequence against that same process-wide state.
...
test('a failed font registration is retried by the next render, not cached', async () => {
  ...
  await expect(renderHtml({ html: '<div>a</div>', width: 200 })).rejects.toThrow(
    'font engine unavailable'
  );
  expect(registerFont).toHaveBeenCalledTimes(4);

  // The next render registers again rather than awaiting the failed promise, and
  // with the engine healthy it renders.
  registerFont.mockClear();
  const second = await renderHtml({ html: '<div>a</div>', width: 200 });
  expect(registerFont).toHaveBeenCalledTimes(4);
