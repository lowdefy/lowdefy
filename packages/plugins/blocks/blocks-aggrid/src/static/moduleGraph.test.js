...
// any renderer pulled in ag-grid or React this import would fail to load.
import * as staticRenderers from './index.js';

const STATIC_DIR = path.join(process.cwd(), 'src', 'static');

test('the static entry exports a toReport renderer for each AgGrid block type', () => {
  const types = [
    'AgGridAlpine',
    'AgGridBalham',
    'AgGridMaterial',
...
test('no renderer file imports ag-grid, React, or a block component', () => {
  const files = fs
