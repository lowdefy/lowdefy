...
// AgGridInputAlpine renders a div with id={blockId} and class ag-theme-alpine,
...
test.describe('AgGridInputAlpine Block', () => {
  test.beforeEach(async ({ page }) => {
    await navigateToTestPage(page, 'aggridinputalpine');
  });
  ...
  test('renders basic grid', async ({ page }) => {
    ...
    const grid = getGrid(page, 'aggridinputalpine_basic');
  ...
  test('renders column headers', async ({ page }) => {
    const headers = getHeaderCells(page, 'aggridinputalpine_basic');
  ...
  test('renders row data from value', async ({ page }) => {
    const rows = getRows(page, 'aggridinputalpine_basic');
  ...
  test('renders with custom height', async ({ page }) => {
    const block = getBlock(page, 'aggridinputalpine_height');
    await expect(block).toBeVisible();
  ...
  test('renders with default column definitions', async ({ page }) => {
    const block = getBlock(page, 'aggridinputalpine_default_col_def');
    await expect(block).toBeVisible();
    const headers = getHeaderCells(page, 'aggridinputalpine_default_col_def');
    await expect(headers).toHaveCount(2);
  });
  ...
  test('onRowClick event fires when row is clicked', async ({ page }) => {
    const block = getBlock(page, 'aggridinputalpine_onrowclick');
    const firstRow = block.locator('.ag-row[row-index="0"] .ag-cell').first();
    await firstRow.click();
    const display = getBlock(page, 'aggridinputalpine_row_click_display');
    await expect(display).toHaveText('Row: Alice');
  });

  test('onCellClick event fires when cell is clicked', async ({ page }) => {
    const block = getBlock(page, 'aggridinputalpine_oncellclick');
    const nameCell = block.locator('.ag-row[row-index="1"] .ag-cell').first();
    await nameCell.click();
    const display = getBlock(page, 'aggridinputalpine_cell_click_display');
    await expect(display).toHaveText('Cell: Bob');
  });

  test('onRowSelected event fires when row is selected', async ({ page }) => {
    const block = getBlock(page, 'aggridinputalpine_onrowselected');
    const firstRow = block.locator('.ag-row[row-index="0"] .ag-cell').first();
    await firstRow.click();
    const display = getBlock(page, 'aggridinputalpine_row_selected_display');
  ...
  test('columns can be sorted when sortable is true', async ({ page }) => {
    const block = getBlock(page, 'aggridinputalpine_sortable');
    const nameHeader = block.locator('.ag-header-cell-text').nth(0);
    await nameHeader.click();

    const display = getBlock(page, 'aggridinputalpine_sort_changed_display');
  ...
  test('columns can be filtered when filter is true', async ({ page }) => {
    const block = getBlock(page, 'aggridinputalpine_filterable');
    ...
    const display = getBlock(page, 'aggridinputalpine_filter_changed_display');
    await expect(display).toHaveText('Filter changed');

    const rows = getRows(page, 'aggridinputalpine_filterable');
  ...
  test('cell can be edited when editable is true', async ({ page }) => {
    ...
    const display = getBlock(page, 'aggridinputalpine_cell_value_changed_display');
  ...
  test('onRowDragEnd event fires when row is dragged', async ({ page }) => {
    ...
    const display = getBlock(page, 'aggridinputalpine_row_drag_display');
  ...
  test('onCellValueChanged captures old value', async ({ page }) => {
    ...
    const display = getBlock(page, 'aggridinputalpine_old_value_display');
