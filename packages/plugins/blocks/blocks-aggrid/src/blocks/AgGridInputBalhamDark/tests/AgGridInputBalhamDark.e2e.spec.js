...
// AgGridInputBalhamDark renders a div with id={blockId} and class ag-theme-balham-dark,
...
test.describe('AgGridInputBalhamDark Block', () => {
  test.beforeEach(async ({ page }) => {
    await navigateToTestPage(page, 'aggridinputbalhamdark');
  });
  ...
  test('renders basic grid', async ({ page }) => {
    const block = getBlock(page, 'aggridinputbalhamdark_basic');
    await expect(block).toBeVisible();
    const grid = getGrid(page, 'aggridinputbalhamdark_basic');
  ...
  test('renders column headers', async ({ page }) => {
    const headers = getHeaderCells(page, 'aggridinputbalhamdark_basic');
  ...
  test('renders row data from value', async ({ page }) => {
    const rows = getRows(page, 'aggridinputbalhamdark_basic');
  ...
  test('renders with custom height', async ({ page }) => {
    const block = getBlock(page, 'aggridinputbalhamdark_height');
    await expect(block).toBeVisible();
  ...
  test('renders with default column definitions', async ({ page }) => {
    const block = getBlock(page, 'aggridinputbalhamdark_default_col_def');
    await expect(block).toBeVisible();
    const headers = getHeaderCells(page, 'aggridinputbalhamdark_default_col_def');
    await expect(headers).toHaveCount(2);
  });
  ...
  test('onRowClick event fires when row is clicked', async ({ page }) => {
    const block = getBlock(page, 'aggridinputbalhamdark_onrowclick');
    const firstRow = block.locator('.ag-row[row-index="0"] .ag-cell').first();
    await firstRow.click();
    const display = getBlock(page, 'aggridinputbalhamdark_row_click_display');
    await expect(display).toHaveText('Row: Alice');
  });

  test('onCellClick event fires when cell is clicked', async ({ page }) => {
    const block = getBlock(page, 'aggridinputbalhamdark_oncellclick');
    const nameCell = block.locator('.ag-row[row-index="1"] .ag-cell').first();
    await nameCell.click();
    const display = getBlock(page, 'aggridinputbalhamdark_cell_click_display');
    await expect(display).toHaveText('Cell: Bob');
  });

  test('onRowSelected event fires when row is selected', async ({ page }) => {
    const block = getBlock(page, 'aggridinputbalhamdark_onrowselected');
    const firstRow = block.locator('.ag-row[row-index="0"] .ag-cell').first();
    await firstRow.click();
    const display = getBlock(page, 'aggridinputbalhamdark_row_selected_display');
  ...
  test('columns can be sorted when sortable is true', async ({ page }) => {
    const block = getBlock(page, 'aggridinputbalhamdark_sortable');
    const nameHeader = block.locator('.ag-header-cell-text').nth(0);
    await nameHeader.click();

    const display = getBlock(page, 'aggridinputbalhamdark_sort_changed_display');
  ...
  test('columns can be filtered when filter is true', async ({ page }) => {
    const block = getBlock(page, 'aggridinputbalhamdark_filterable');
    ...
    const display = getBlock(page, 'aggridinputbalhamdark_filter_changed_display');
    await expect(display).toHaveText('Filter changed');

    const rows = getRows(page, 'aggridinputbalhamdark_filterable');
  ...
  test('cell can be edited when editable is true', async ({ page }) => {
    const block = getBlock(page, 'aggridinputbalhamdark_editable');
    ...
    const display = getBlock(page, 'aggridinputbalhamdark_cell_value_changed_display');
  ...
  test('row drag handle is visible when rowDrag is true', async ({ page }) => {
    const block = getBlock(page, 'aggridinputbalhamdark_rowdrag');
  ...
  test('onRowDragEnd event fires when row is dragged', async ({ page }) => {
    ...
    const display = getBlock(page, 'aggridinputbalhamdark_row_drag_display');
  ...
  test('onCellValueChanged captures old value', async ({ page }) => {
    ...
    const display = getBlock(page, 'aggridinputbalhamdark_old_value_display');
