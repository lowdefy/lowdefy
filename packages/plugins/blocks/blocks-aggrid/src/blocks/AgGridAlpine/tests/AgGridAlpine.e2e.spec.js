...
// AgGridAlpine renders a div with id={blockId} and class ag-theme-alpine,
...
test.describe('AgGridAlpine Block', () => {
  test.beforeEach(async ({ page }) => {
    await navigateToTestPage(page, 'aggridalpine');
  });
  ...
  test('renders basic grid', async ({ page }) => {
    const block = getBlock(page, 'aggridalpine_basic');
    await expect(block).toBeVisible();
    const grid = getGrid(page, 'aggridalpine_basic');
  ...
  test('renders column headers', async ({ page }) => {
    const headers = getHeaderCells(page, 'aggridalpine_basic');
  ...
  test('renders row data', async ({ page }) => {
    const rows = getRows(page, 'aggridalpine_basic');
  ...
  test('renders with custom height', async ({ page }) => {
    const block = getBlock(page, 'aggridalpine_height');
    await expect(block).toBeVisible();
  ...
  test('renders with default column definitions', async ({ page }) => {
    const block = getBlock(page, 'aggridalpine_default_col_def');
    await expect(block).toBeVisible();
    const headers = getHeaderCells(page, 'aggridalpine_default_col_def');
    await expect(headers).toHaveCount(2);
  });

  test('renders empty grid with no rows', async ({ page }) => {
    const grid = getGrid(page, 'aggridalpine_empty');
    await expect(grid).toBeVisible();
    const overlay = getBlock(page, 'aggridalpine_empty').locator('.ag-overlay-no-rows-center');
  ...
  test('columns can be sorted when sortable is true', async ({ page }) => {
    const block = getBlock(page, 'aggridalpine_sortable');
  ...
  test('columns can be filtered when filter is true', async ({ page }) => {
    const block = getBlock(page, 'aggridalpine_filterable');
