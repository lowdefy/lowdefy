...
// AgGridLowdefy renders a div with id={blockId} carrying the shared antd helper class and the
// avatar size custom properties, containing the AgGridReact component. It carries no file-theme
// wrapper class — the grid is themed by the Theming API object passed as the theme grid option.
...
test.describe('AgGridLowdefy Block', () => {
  ...
  test('renders basic grid', async ({ page }) => {
    const block = getBlock(page, 'aggridlowdefy_basic');
    await expect(block).toBeVisible();
    const grid = getGrid(page, 'aggridlowdefy_basic');
    await expect(grid).toBeVisible();
  });

  test('renders column headers', async ({ page }) => {
    const headers = getHeaderCells(page, 'aggridlowdefy_basic');
  ...
  test('renders row data', async ({ page }) => {
    const rows = getRows(page, 'aggridlowdefy_basic');
  ...
  test('renders with custom height', async ({ page }) => {
    const block = getBlock(page, 'aggridlowdefy_height');
    await expect(block).toBeVisible();
  ...
  test('renders with default column definitions', async ({ page }) => {
    const block = getBlock(page, 'aggridlowdefy_default_col_def');
    await expect(block).toBeVisible();
    const headers = getHeaderCells(page, 'aggridlowdefy_default_col_def');
    await expect(headers).toHaveCount(2);
  });

  test('renders empty grid with no rows', async ({ page }) => {
    const grid = getGrid(page, 'aggridlowdefy_empty');
    await expect(grid).toBeVisible();
    const overlay = getBlock(page, 'aggridlowdefy_empty').locator('.ag-overlay-no-rows-center');
  ...
  test('onRowClick event fires when row is clicked', async ({ page }) => {
    const block = getBlock(page, 'aggridlowdefy_onrowclick');
    const firstRow = block.locator('.ag-row[row-index="0"] .ag-cell').first();
    await firstRow.click();
    const display = getBlock(page, 'aggridlowdefy_row_click_display');
  ...
  test('onCellClick event fires when cell is clicked', async ({ page }) => {
    ...
    const display = getBlock(page, 'aggridlowdefy_cell_click_display');
    await expect(display).toHaveText('Cell: Bob');
  });

  test('onRowSelected event fires when row is selected', async ({ page }) => {
    const block = getBlock(page, 'aggridlowdefy_onrowselected');
    const firstRow = block.locator('.ag-row[row-index="0"] .ag-cell').first();
    await firstRow.click();
    const display = getBlock(page, 'aggridlowdefy_row_selected_display');
  ...
  test('columns can be sorted when sortable is true', async ({ page }) => {
    const block = getBlock(page, 'aggridlowdefy_sortable');
    // Click Name header to sort ascending
    const nameHeader = block.locator('.ag-header-cell-text').nth(0);
    await nameHeader.click();

    // Verify sort changed event fired
    const display = getBlock(page, 'aggridlowdefy_sort_changed_display');
  ...
  test('columns can be filtered when filter is true', async ({ page }) => {
    const block = getBlock(page, 'aggridlowdefy_filterable');
    ...
    const display = getBlock(page, 'aggridlowdefy_filter_changed_display');
    await expect(display).toHaveText('Filter changed');

    // Verify filtered results
    const rows = getRows(page, 'aggridlowdefy_filterable');
  ...
  test('cell.type: tag with colorFrom reads colour from row data path', async ({ page }) => {
    const block = getBlock(page, 'aggridlowdefy_cell_tag_from');
  ...
  test('cell.type: tag renders one styled span per array item', async ({ page }) => {
    const block = getBlock(page, 'aggridlowdefy_cell_tag_array');
  ...
  test('cell.type: tag auto-colours unmapped values with the seeded styled-tag pattern', async ({
    ...
    const block = getBlock(page, 'aggridlowdefy_cell_tag_seeded');
    const firstRowSpan = block.locator('.ag-row[row-index="0"] .ag-cell span').first();
  ...
  test('cell.type: avatar renders picture when srcField is present', async ({ page }) => {
    const block = getBlock(page, 'aggridlowdefy_cell_avatar');
  ...
  test('cell.type: link click emits onCellLink with resolved urlQuery', async ({ page }) => {
    ...
    await anchor.click();
    const display = getBlock(page, 'aggridlowdefy_link_display');
  ...
  test('cell.type: boolean renders default Yes/No labels', async ({ page }) => {
    const block = getBlock(page, 'aggridlowdefy_cell_boolean');
  ...
  test('cell.type: progress renders percent with suffix', async ({ page }) => {
    const block = getBlock(page, 'aggridlowdefy_cell_progress');
  ...
  test('cell.type: number with format currency renders with symbol', async ({ page }) => {
    const block = getBlock(page, 'aggridlowdefy_cell_number');
  ...
  test('cell.align: left override wins over auto right-align', async ({ page }) => {
    const block = getBlock(page, 'aggridlowdefy_cell_align_override');
    const cell = block.locator('.ag-row[row-index="0"] .ag-cell').first();
  ...
  test('ellipsis: N installs a line-clamp renderer', async ({ page }) => {
    const block = getBlock(page, 'aggridlowdefy_ellipsis');
  ...
  test('cell.type: buttons click triggers the named block event', async ({ page }) => {
    ...
    const display = getBlock(page, 'aggridlowdefy_button_display');
    await expect(display).toHaveText('Edit: TSK-001');
  });
  ...
  // rules never match it. These two assertions are what prove the inline --lf-avatar-size /
  ...
  test('loading flag toggles loading overlay at runtime', async ({ page }) => {
    const block = getBlock(page, 'aggridlowdefy_loading_grid');
    ...
    const toggle = getBlock(page, 'aggridlowdefy_loading_toggle').locator('button').first();
    await toggle.click();
    await expect(overlay).toBeVisible();
