...
// AgGridMaterial renders a div with id={blockId} and class ag-theme-material,
...
test.describe('AgGridMaterial Block', () => {
  test.beforeEach(async ({ page }) => {
    await navigateToTestPage(page, 'aggridmaterial');
  });
  ...
  test('renders basic grid', async ({ page }) => {
    const block = getBlock(page, 'aggridmaterial_basic');
    await expect(block).toBeVisible();
    const grid = getGrid(page, 'aggridmaterial_basic');
  ...
  test('renders column headers', async ({ page }) => {
    const headers = getHeaderCells(page, 'aggridmaterial_basic');
  ...
  test('renders row data', async ({ page }) => {
    const rows = getRows(page, 'aggridmaterial_basic');
  ...
  test('renders with custom height', async ({ page }) => {
    const block = getBlock(page, 'aggridmaterial_height');
    await expect(block).toBeVisible();
  ...
  test('renders with default column definitions', async ({ page }) => {
    const block = getBlock(page, 'aggridmaterial_default_col_def');
    await expect(block).toBeVisible();
    const headers = getHeaderCells(page, 'aggridmaterial_default_col_def');
    await expect(headers).toHaveCount(2);
  });

  test('renders empty grid with no rows', async ({ page }) => {
    const grid = getGrid(page, 'aggridmaterial_empty');
    await expect(grid).toBeVisible();
    const overlay = getBlock(page, 'aggridmaterial_empty').locator('.ag-overlay-no-rows-center');
  ...
  test('onRowClick event fires when row is clicked', async ({ page }) => {
    const block = getBlock(page, 'aggridmaterial_onrowclick');
    const firstRow = block.locator('.ag-row[row-index="0"] .ag-cell').first();
    await firstRow.click();
    const display = getBlock(page, 'aggridmaterial_row_click_display');
    await expect(display).toHaveText('Row: Alice');
  });

  test('onCellClick event fires when cell is clicked', async ({ page }) => {
    const block = getBlock(page, 'aggridmaterial_oncellclick');
    const nameCell = block.locator('.ag-row[row-index="1"] .ag-cell').first();
    await nameCell.click();
    const display = getBlock(page, 'aggridmaterial_cell_click_display');
    await expect(display).toHaveText('Cell: Bob');
  });

  test('onRowSelected event fires when row is selected', async ({ page }) => {
    const block = getBlock(page, 'aggridmaterial_onrowselected');
    const firstRow = block.locator('.ag-row[row-index="0"] .ag-cell').first();
    await firstRow.click();
    const display = getBlock(page, 'aggridmaterial_row_selected_display');
  ...
  test('columns can be sorted when sortable is true', async ({ page }) => {
    const block = getBlock(page, 'aggridmaterial_sortable');
    // Click Name header to sort ascending
    const nameHeader = block.locator('.ag-header-cell-text').nth(0);
    await nameHeader.click();

    // Verify sort changed event fired
    const display = getBlock(page, 'aggridmaterial_sort_changed_display');
  ...
  test('columns can be filtered when filter is true', async ({ page }) => {
    const block = getBlock(page, 'aggridmaterial_filterable');
    ...
    const display = getBlock(page, 'aggridmaterial_filter_changed_display');
    await expect(display).toHaveText('Filter changed');

    // Verify filtered results
    const rows = getRows(page, 'aggridmaterial_filterable');
  ...
  test('cell.type: tag renders on Material with color-mix styling', async ({ page }) => {
    const block = getBlock(page, 'aggridmaterial_cell_types');
  ...
  test('cell.type: avatar renders initials on Material', async ({ page }) => {
    const block = getBlock(page, 'aggridmaterial_cell_types');
  ...
  test('cell.type: number right-aligns on Material', async ({ page }) => {
    const block = getBlock(page, 'aggridmaterial_cell_types');
