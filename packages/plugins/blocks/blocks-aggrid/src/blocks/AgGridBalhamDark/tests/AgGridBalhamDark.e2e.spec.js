...
// AgGridBalhamDark renders a div with id={blockId} and class ag-theme-balham-dark,
...
test.describe('AgGridBalhamDark Block', () => {
  test.beforeEach(async ({ page }) => {
    await navigateToTestPage(page, 'aggridbalhamdark');
  });
  ...
  test('renders basic grid', async ({ page }) => {
    ...
    const grid = getGrid(page, 'aggridbalhamdark_basic');
  ...
  test('renders column headers', async ({ page }) => {
    const headers = getHeaderCells(page, 'aggridbalhamdark_basic');
  ...
  test('renders row data', async ({ page }) => {
    const rows = getRows(page, 'aggridbalhamdark_basic');
  ...
  test('renders with custom height', async ({ page }) => {
    const block = getBlock(page, 'aggridbalhamdark_height');
    await expect(block).toBeVisible();
  ...
  test('renders with default column definitions', async ({ page }) => {
    const block = getBlock(page, 'aggridbalhamdark_default_col_def');
    await expect(block).toBeVisible();
    const headers = getHeaderCells(page, 'aggridbalhamdark_default_col_def');
    await expect(headers).toHaveCount(2);
  });

  test('renders empty grid with no rows', async ({ page }) => {
    const grid = getGrid(page, 'aggridbalhamdark_empty');
    await expect(grid).toBeVisible();
    const overlay = getBlock(page, 'aggridbalhamdark_empty').locator('.ag-overlay-no-rows-center');
  ...
  test('onRowClick event fires when row is clicked', async ({ page }) => {
    const block = getBlock(page, 'aggridbalhamdark_onrowclick');
    const firstRow = block.locator('.ag-row[row-index="0"] .ag-cell').first();
    await firstRow.click();
    const display = getBlock(page, 'aggridbalhamdark_row_click_display');
    await expect(display).toHaveText('Row: Alice');
  });

  test('onCellClick event fires when cell is clicked', async ({ page }) => {
    const block = getBlock(page, 'aggridbalhamdark_oncellclick');
    const nameCell = block.locator('.ag-row[row-index="1"] .ag-cell').first();
    await nameCell.click();
    const display = getBlock(page, 'aggridbalhamdark_cell_click_display');
    await expect(display).toHaveText('Cell: Bob');
  });

  test('onRowSelected event fires when row is selected', async ({ page }) => {
    const block = getBlock(page, 'aggridbalhamdark_onrowselected');
    const firstRow = block.locator('.ag-row[row-index="0"] .ag-cell').first();
    await firstRow.click();
    const display = getBlock(page, 'aggridbalhamdark_row_selected_display');
  ...
  test('columns can be sorted when sortable is true', async ({ page }) => {
    const block = getBlock(page, 'aggridbalhamdark_sortable');
    // Click Name header to sort ascending
    const nameHeader = block.locator('.ag-header-cell-text').nth(0);
    await nameHeader.click();

    // Verify sort changed event fired
    const display = getBlock(page, 'aggridbalhamdark_sort_changed_display');
  ...
  test('columns can be filtered when filter is true', async ({ page }) => {
    const block = getBlock(page, 'aggridbalhamdark_filterable');
    ...
    const display = getBlock(page, 'aggridbalhamdark_filter_changed_display');
    await expect(display).toHaveText('Filter changed');

    // Verify filtered results
    const rows = getRows(page, 'aggridbalhamdark_filterable');
