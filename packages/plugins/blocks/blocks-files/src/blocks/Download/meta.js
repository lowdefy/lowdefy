...
export default {
  ...
  properties: {
    type: 'object',
    required: ['downloadPolicyRequestId'],
    properties: {
      downloadPolicyRequestId: {
        type: 'string',
        description:
          'Id of a download request (e.g. AwsS3PresignedGetObject, GcsSignedGetUrl, AzureBlobDownloadSas) that returns a URL for the file to download.',
      },
      fileList: {
        type: 'array',
        description:
          'List of files to be downloaded. If files were uploaded using an Upload block, the fileList value can just be mapped to this field.',
        items: {
          type: 'object',
          required: ['key'],
          properties: {
