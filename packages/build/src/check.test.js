...
test('a dev-stage validateOnly build reports the same prodError warning as a warning', async () => {
  const app = createApp({ lowdefyYaml: missingMenuPageApp });
  created.push(app.root);
...
test('a build without validateOnly still writes the build directory', async () => {
  const app = createApp({ lowdefyYaml: cleanApp });
  created.push(app.root);
  await build(app.options);
  expect(fs.readdirSync(app.buildDirectory)).not.toEqual([]);
});
