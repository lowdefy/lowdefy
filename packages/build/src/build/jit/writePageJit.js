...
import { walkBlocks } from '../collectTailwindClasses.js';
...
async function writePageJit({ page, context }) {
  ...
  // Collect Tailwind classes from the JIT page and merge into tailwind-classes.js
  const pageClasses = new Set();
  walkBlocks([page], pageClasses);
  if (pageClasses.size > 0) {
    if (!context.tailwindClasses) {
      context.tailwindClasses = new Set();
    }
    let changed = false;
    for (const cls of pageClasses) {
      if (!context.tailwindClasses.has(cls)) {
        context.tailwindClasses.add(cls);
        changed = true;
      }
    }
    if (changed) {
  ...
  // Write per-page content file for Tailwind to scan property strings
  const pageContent = collectPageContent([page]);
  await context.writeBuildArtifact(
    `tailwind-content/${page.pageId}.html`,
