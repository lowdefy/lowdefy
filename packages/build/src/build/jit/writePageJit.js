...
async function writePageJit({ page, context, tailwindContent }) {
  ...
  // candidates inside _js source are included. Skipping unchanged content lets
  // the caller avoid touching the CSS recompilation trigger
  // (tailwind-candidates.css) for builds that introduced no new class candidates.
  const tailwindChanged = await writeFileIfChanged(
    path.join(
      context.directories.server,
      'lowdefy-build',
      'tailwind',
      `${encodeURIComponent(page.pageId)}.html`
    ),
    '<!-- Generated by Lowdefy build -->\n' + (tailwindContent ?? '')
  );
  return { tailwindChanged };
}
