...
async function writePageJit({ page, context }) {
  ...
  const newContent = '<!-- Generated by Lowdefy build -->\n' + (pageContent ?? '');
  // Only write if content changed to avoid retriggering the tailwind content watcher
  const existing = await readFile(tailwindPath);
  if (existing !== newContent) {
    await writeFile(tailwindPath, newContent);
  }
}
