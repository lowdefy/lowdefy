...
test('buildAgents throws when transcription.connectionId is not a string', () => {
  const context = testContext();
  const components = {
    connections: [{ id: 'connection:conn1', connectionId: 'conn1', type: 'Anthropic' }],
    agents: [
      {
        ...
        properties: {
          model: 'test-model',
          transcription: { connectionId: 123, model: 'whisper-1' },
        },
      },
    ],
  };
  expect(() => buildAgents({ components, context })).toThrow(
    'Agent "agent1" "transcription.connectionId" is not a string.'
  );
});

test('buildAgents throws when transcription.connectionId references a missing connection', () => {
  const context = testContext();
  const components = {
    connections: [{ id: 'connection:conn1', connectionId: 'conn1', type: 'Anthropic' }],
    agents: [
      {
        ...
        properties: {
          model: 'test-model',
          transcription: { connectionId: 'missing-conn', model: 'whisper-1' },
        },
      },
    ],
  };
  expect(() => buildAgents({ components, context })).toThrow(
    'Agent "agent1" "transcription.connectionId" references connection "missing-conn" which does not exist.'
  );
});

test('buildAgents throws when transcription.model is not defined', () => {
  const context = testContext();
  const components = {
    connections: [{ id: 'connection:conn1', connectionId: 'conn1', type: 'Anthropic' }],
    agents: [
      {
        ...
        properties: {
          model: 'test-model',
          transcription: { connectionId: 'conn1' },
        },
      },
    ],
  };
  expect(() => buildAgents({ components, context })).toThrow(
    'Agent "agent1" "transcription.model" is not defined.'
  );
});

test('buildAgents passes with valid transcription config matching connection by connectionId', () => {
  const context = testContext();
  const components = {
    connections: [
      { id: 'connection:conn1', connectionId: 'conn1', type: 'Anthropic' },
      { id: 'connection:openai', connectionId: 'openai', type: 'OpenAI' },
    ],
    agents: [
      {
        id: 'agent1',
  ...
  expect(() => buildAgents({ components, context })).not.toThrow();
  expect(components.agents[0].properties.transcription).toEqual({
    connectionId: 'openai',
    model: 'whisper-1',
  });
});
