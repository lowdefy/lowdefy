...
test('writeGlobalsCss includes layer order and tailwind import', async () => {
  ...
  expect(css).toContain('@source "./tailwind-content/*.html";');
...
test('writeGlobalsCss includes @source for tailwind-content/*.html', async () => {
  const context = createContext();
  await writeGlobalsCss({ components: {}, context });
...
test('writeGlobalsCss writes per-page content files from tailwindContentMap', async () => {
  const context = createContext();
  context.tailwindContentMap = new Map([
    ['home', '<div class="bg-red-500">Hello</div>'],
    ['about', '<p class="text-lg">About</p>'],
  ]);
  await writeGlobalsCss({ components: {}, context });

  const homeCall = context.writeBuildArtifact.mock.calls.find(
    (call) => call[0] === 'tailwind-content/home.html'
  );
  expect(homeCall).toBeDefined();
  expect(homeCall[1]).toContain('<div class="bg-red-500">Hello</div>');
  expect(homeCall[1]).toContain('<!-- Generated by Lowdefy build -->');

  const aboutCall = context.writeBuildArtifact.mock.calls.find(
    (call) => call[0] === 'tailwind-content/about.html'
  );
...
test('writeGlobalsCss writes no content files when tailwindContentMap is undefined', async () => {
  ...
  const contentCalls = context.writeBuildArtifact.mock.calls.filter(
    (call) => call[0].startsWith('tailwind-content/')
  );
