...
test('writeGlobalsCss writes per-page content files to server directory', async () => {
  ...
  const homeCall = mockWriteFile.mock.calls.find((call) =>
    call[0].endsWith('/lowdefy-build/tailwind/home.html')
  );
  ...
  const aboutCall = mockWriteFile.mock.calls.find((call) =>
    call[0].endsWith('/lowdefy-build/tailwind/about.html')
  );
